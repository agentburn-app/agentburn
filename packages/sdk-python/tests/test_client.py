...
from agentburn import AgentBurn, AgentBurnError, idempotency_key
from agentburn.http import HttpResponse
...
class TestTrack(unittest.TestCase):
    def test_track_posts_one_event_and_returns_result(self):
        ...
        call = fake.calls[0]
        self.assertEqual(call["url"], "https://agentburn.dev/api/ingest")
    ...
    def test_honors_custom_base_url(self):
        fake = FakeHttpClient()
        fake.responses.append(ok_json([{"accepted": True}]))
        ab = AgentBurn("k", base_url="https://selfhost.example/", http=fake)
        ab.track({"provider": "x"})
    ...
    def test_throws_on_401_with_server_message(self):
        ...
        ab = AgentBurn("bad", http=fake)
        with self.assertRaises(AgentBurnError) as ctx:
            ab.track({"provider": "x"})
    ...
    def test_retries_on_429_then_succeeds(self):
        ...
        fake.responses.append(ok_json([{"accepted": True}]))
        ab = AgentBurn("k", retries=2, http=fake)
        result = ab.track({"provider": "x"})
        self.assertTrue(result["accepted"])
        self.assertEqual(len(fake.calls), 2)

    def test_requires_api_key(self):
        with self.assertRaises(AgentBurnError):
            AgentBurn("")
