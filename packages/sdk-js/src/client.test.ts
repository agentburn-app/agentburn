...
import { AgentBurn, AgentBurnError } from "./index";
...
describe("AgentBurn.track", () => {
  it("posts one event and returns the result", async () => {
    ...
    expect(url).toBe("https://agentburn.dev/api/ingest");
  ...
  it("honors a custom baseUrl", async () => {
    const { calls, respond } = captureFetch();
    const ab = new AgentBurn({ apiKey: "k", baseUrl: "https://selfhost.example/" });
  ...
  it("throws AgentBurnError on 401 with the server message", async () => {
    const { respond } = captureFetch();
    const ab = new AgentBurn({ apiKey: "bad" });
    const p = ab.track({ provider: "x" });
    respond(jsonResponse({ error: "Unauthorized" }, 401));
    await expect(p).rejects.toThrow(AgentBurnError);
    await expect(p).rejects.toThrow("Unauthorized");
  });

  it("retries on 429 then succeeds", async () => {
    const { calls, respond } = captureFetch();
    const ab = new AgentBurn({ apiKey: "k", retries: 2 });
    const p = ab.track({ provider: "x" });
