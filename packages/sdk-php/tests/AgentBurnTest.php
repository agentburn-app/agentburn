 ...
 *
 * Run:  php tests/AgentBurnTest.php
 * Exits non-zero on the first failure.
 */

require __DIR__ . '/../src/AgentBurnException.php';
require __DIR__ . '/../src/HttpClient.php';
require __DIR__ . '/../src/Idempotency.php';
require __DIR__ . '/../src/AgentBurn.php';

use AgentBurn\AgentBurn;
use AgentBurn\AgentBurnException;
use AgentBurn\HttpClient;
use AgentBurn\HttpResponse;
...
$ab = new AgentBurn('k', 'https://agentburn.dev', 3, $fake);
...
same('https://agentburn.dev/api/ingest', $fake->calls[0]['url'], 'posts to /api/ingest');
...
// 3. custom baseUrl
$fake = new FakeHttpClient();
$ab = new AgentBurn('k', 'https://selfhost.example/', 3, $fake);
$fake->responses[] = okJson([['accepted' => true]]);
$ab->track(['provider' => 'x']);
...
$fake = new FakeHttpClient();
$ab = new AgentBurn('bad', 'https://agentburn.dev', 3, $fake);
$fake->responses[] = new HttpResponse(401, json_encode(['error' => 'Unauthorized'], JSON_THROW_ON_ERROR));
try {
    $ab->track(['provider' => 'x']);
    check('throws AgentBurnException on 401', false);
} catch (AgentBurnException $e) {
    same('Unauthorized', $e->getMessage(), 'throws AgentBurnException with server message');
}

// 5. retries on 429 then succeeds
$fake = new FakeHttpClient();
$ab = new AgentBurn('k', 'https://agentburn.dev', 2, $fake);
